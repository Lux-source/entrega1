...
export class ClienteCarro extends Presenter {
    ...
    template() {
        ...
        const items = carro.map(item => {
            const libro = this.model.libros.find(l => l.id === item.libroId);
            return { ...item, libro };
        }).filter(item => item.libro);

        const total = items.reduce((sum, item) => sum + (item.libro.precio * item.cantidad), 0);
            ...
            <div class="carro-container">
                ...
                <div class="carro-content">
                        ...
                        ${items.map((item, index) => `
                            <div class="carro-item">
                                ...
                                <div class="item-subtotal">
                                    ${(item.libro.precio * item.cantidad).toFixed(2)}€
                                </div>
                    ...
                    <div class="carro-resumen">
                        ...
                        <button id="btn-finalizar" class="btn btn-primary btn-block">
                            Finalizar Compra
                        </button>
    ...
    bind() {
        ...
        // Finalizar compra
        const btnFinalizar = this.container.querySelector('#btn-finalizar');
        if (btnFinalizar) {
    ...
    actualizarCantidad(index, action) {
        ...
        if (action === 'increase') {
            const libro = this.model.libros.find(l => l.id === carro[index].libroId);
    ...
    finalizarCompra() {
        ...
        session.pushSuccess('¡Compra realizada con éxito!');
