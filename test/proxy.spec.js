...
describe("Cálculos", () => {
	...
	it("calcula el total de la compra mediante el proxy", async () => {
		const libros = await libreriaProxy.getLibros();
		const libro1 = libros[0];
		const libro2 = libros[1];
		...
		const totalEsperado = libro1.precio * 3 + libro2.precio * 1;
		expect(compra.id).to.be.a("string").with.lengthOf(24);
		expect(compra.numero).to.be.a("string").and.to.match(/^FAC-/);
		expect(compra.fecha).to.be.a("string");
		expect(compra.clienteId).to.equal(clienteId);
		expect(compra.total).to.be.closeTo(totalEsperado, 0.1);

		expect(compra.items).to.be.an("array").with.lengthOf(2);
		expect(compra.items[0].libroId).to.equal(libro1.id);
		expect(compra.items[0].cantidad).to.equal(3);
		expect(compra.items[1].libroId).to.equal(libro2.id);
		expect(compra.items[1].cantidad).to.equal(1);

		expect(compra.envio).to.be.an("object");
		expect(compra.envio.nombre).to.equal("Test");
		expect(compra.envio.direccion).to.equal("Test Dir");
		expect(compra.envio.ciudad).to.equal("Test City");
		expect(compra.envio.cp).to.equal("00000");
		expect(compra.envio.telefono).to.equal("600000000");
	});

	it("verifica que el stock se reduce tras la compra mediante el proxy", async () => {
		const libros = await libreriaProxy.getLibros();
		const libro = libros[2];
